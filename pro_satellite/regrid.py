...
def drop_in_the_box(grid_dict, sat_lat, sat_lon, sat_var,
    ...
    """ Regrid satellite data through drop in the box.
        drop_in_the_box processes 2D satellite data
        drop_in_the_box_1D_sat processes 1D satellite data
    (ywang, 04/10/20)
...
#------------------------------------------------------------------------------
#
def drop_in_the_box_1D_sat(grid_dict, sat_lat, sat_lon, sat_var):
    """ Regrid satellite data through drop in the box.
    drop_in_the_box processes 2D satellite data
    drop_in_the_box_1D_sat processes 1D satellite data
    (ywang, 08/26/2023)
    ...
    grid_dict : dict
        ...
        'PlateCarree': Starting point, Ending point, and Step
            are given. Keys are 'lat_start', 'lat_end',
            lat_step', 'lon_start', 'lon_end' and 'lon_step'.
    sat_lat : 1-D numpy array
        Satellite latitude
    sat_lon : 1-D numpy array
        Satellite longitude
    sat_var : 1-D numpy array
        Satellite variable array to be (re)grided.
    ...
    if (grid_dict['proj'].lower() == 'PlateCarree'.lower()):
        ...
        # map satellite pixels to grid box
        sat_grid_ii = np.zeros( (sat_n,), dtype=int )
        sat_grid_jj = np.zeros( (sat_n,), dtype=int )
        for sat_ind in range(sat_n):
            ...
            # save index
            sat_grid_ii[sat_ind] = grid_i
            sat_grid_jj[sat_ind] = grid_j
    ...
    # array to save satellite data
    sat_grid = np.zeros((n_lat,n_lon), dtype=float)
    ...
    for sat_ind in range(sat_n):
        ...
        grid_i = sat_grid_ii[sat_ind]
        grid_j = sat_grid_jj[sat_ind]

        # accumulate satellite data at new grid
        sat_grid[grid_i,grid_j] += sat_var[sat_ind]

        # number of satellite data in a grid
        sat_grid_count[grid_i,grid_j] += 1

    # average
    sat_grid_flag = (sat_grid_count > 0)
    sat_grid[sat_grid_flag] /= sat_grid_count[sat_grid_flag]
    sat_grid[np.logical_not(sat_grid_flag)] = np.nan
